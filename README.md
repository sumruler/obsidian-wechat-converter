...
现在，Wechat Converter 不再只服务于微信公众号。你可以在同一个“发布与分发”窗口里继续把文章发送到知乎、掘金、CSDN、语雀、小红书等平台，由 Obsidian 发布助手浏览器插件接管真实的浏览器登录态和草稿保存流程。
...
> 多平台分发仍是 Beta 能力。各平台的草稿保存、风控、登录状态和失败重试由 Obsidian 发布助手浏览器插件处理；Obsidian 侧负责写作、排版、平台选择和任务投递。
...
1.  **🌐 多平台发布 (Multi-platform Publishing) ⭐ Beta**
    ...
    - **Obsidian 发布助手浏览器插件接管平台登录态**：其他平台通过浏览器已有登录态保存草稿，Obsidian 不内嵌平台登录，也不接管 Cookie。
    - **微信与其他平台分工清晰**：微信公众号继续使用插件自己的官方 API 链路，其他平台走 Obsidian 发布助手浏览器插件链路。
...
- **多平台发布进入发布窗口**：现在“发布与分发”窗口支持切换到“其他平台”，把文章发送到 Obsidian 发布助手浏览器插件，再由插件保存到知乎、掘金、CSDN 等平台草稿箱。
- **Obsidian 发布助手 桥接设置更完整**：设置页新增多平台分发配置、Token 校验、平台选择、连接测试和已选平台登录状态检测。
